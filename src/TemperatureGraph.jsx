...
const TemperatureGraph = ({ data, timeframe }) => {
    const [filteredData, setFilteredData] = useState([]);
  
    useEffect(() => {
      const getRecentData = (timeFrame) => {
        const now = new Date();
      ...
      const filtered = getRecentData(timeframe);
    ...
    <div className="temperature-graph-container">
        <ResponsiveContainer width="150%" height={150}>
          <LineChart data={filteredData}>
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="insideTemperature" name="Inside Temp" stroke="#8884d8" />
            <Line type="monotone" dataKey="outsideTemperature" name="Outside Temp" stroke="#82ca9d" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    );
