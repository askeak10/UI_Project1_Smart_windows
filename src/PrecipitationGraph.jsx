...
const PrecipitationGraph = ({ timeframe }) => {
  const [filteredData, setFilteredData] = useState([]);

  useEffect(() => {
    const getRecentPrecipitationData = (timeFrame) => {
      const now = new Date();
    ...
    const filtered = getRecentPrecipitationData(timeframe);
  ...
  return (
    <div className="precipitation-graph-container">
      <ResponsiveContainer width="150%" height={150}>
        <LineChart data={filteredData}>
          ...
          <Tooltip />
          <Legend />
          <Line type="monotone" dataKey="precipitation" name="Precipitation" stroke="#82ca9d" />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
